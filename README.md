...
- Scratch opcode の網羅的な IR 実装（現状はスレッド本体が動き系、見た目の大きさ変更系、変数代入、タイマーリセットのみ。式はリテラル、演算子、変数参照、見た目の大きさレポーター、タイマーレポーターのみ）
...
`run` は現状もっとも実験的なコマンドです。文ブロックは動き系、見た目の大きさ変更系、変数代入、タイマーリセット、入力式はリテラル・演算子・変数参照・大きさレポーター・タイマーレポーターのみを含むシンプルなプロジェクトから試すことを推奨します。成功時は各スレッドの状態が実行中に定期的に標準出力へ表示されます。
