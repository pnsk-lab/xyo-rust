...
| 5 | 一部の式 / 文を LLVM IR へ変換し、JIT で実行する | 🚧 一部実装（文は動き系、見た目の大きさ変更系、変数代入。式はリテラル + 演算子 + 変数参照 + 一部レポーター） |
...
- `run` サブコマンドは実験的で、スレッド本体は動き系文ブロック、見た目の大きさ変更系、変数代入のみ、入力式はリテラル、演算子、変数参照、一部レポーターのみ対応しています
