...
| 5 | 一部の式 / 文を LLVM IR へ変換し、JIT で実行する | 🚧 一部実装（文は動き系、見た目の大きさ変更系、変数代入、タイマーリセット。式はリテラル + 演算子 + 変数参照 + 見た目の大きさレポーター + タイマーレポーター） |
...
- `run` サブコマンドは実験的で、スレッド本体は動き系文ブロック、見た目の大きさ変更系、変数代入、タイマーリセットのみ、入力式はリテラル、演算子、変数参照、見た目の大きさレポーター、タイマーレポーターのみ対応しています
...
![star history](https://api.star-history.com/chart?repos=pnsk-lab/xyo-rust&type=date&legend=top-left)
