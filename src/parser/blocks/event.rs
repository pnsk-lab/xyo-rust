    ...
    match block.opcode {
        BlockOpCodes::EventBroadcast => {
            let inputs = block.inputs.as_ref().ok_or(ParserError::InvalidValue(
                "missing inputs in EventBroadcast block",
            ))?;
            let broadcast = inputs
                .get("BROADCAST_INPUT")
                .ok_or(ParserError::InvalidValue("missing BROADCAST_INPUT input"))?;
            let broadcast = parse_input(project, target_idx, broadcast).map_err(|err| {
                err.context("failed to parse BROADCAST_INPUT in EventBroadcast block")
            })?;
        ...
        BlockOpCodes::EventBroadcastAndWait => {
            ...
            let broadcast = inputs
                ...
                .ok_or(ParserError::InvalidValue("missing BROADCAST_INPUT input"))?;
            let broadcast = parse_input(project, target_idx, broadcast).map_err(|err| {
                err.context("failed to parse BROADCAST_INPUT in EventBroadcastAndWait block")
            })?;
